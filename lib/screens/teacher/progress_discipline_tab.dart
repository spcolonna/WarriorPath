...
class _ProgressDisciplineTabState extends State<ProgressDisciplineTab> {
  ...
  Future<void> _showPromotionDialog(
    ...
    List<QueryDocumentSnapshot> allLevels,
  ) async {
        ...
        builder: (context, setDialogState) {
          return AlertDialog(
            ...
            content: Column(
              ...
              children: [
                DropdownButtonFormField<DocumentSnapshot>(
                  hint: Text(l10n.choseNewLevel),
                  initialValue: selectedNextLevel,
                  items: allLevels
                      .map(
                        (levelDoc) => DropdownMenuItem<DocumentSnapshot>(
                          value: levelDoc,
            ...
            actions: [
              ...
              ElevatedButton(
                onPressed: selectedNextLevel == null
                    ? null
                    : () {
                        _promoteStudent(
  ...
  Future<void> _deletePromotionHistory(String historyId) async {
    ...
    try {
      ...
      final newLatestPromoSnap = await memberRef
          .collection('progressionHistory')
      ...
      if (newLatestPromoSnap.docs.isNotEmpty) {
        ...
        final levelsSnap = await firestore
            .collection('schools')
            .doc(widget.schoolId)
            .collection('disciplines')
            .doc(widget.discipline.id)
              ...
              children: [
                Row(
                  ...
                  children: [
                    ...
                    Chip(
                      label: Text(
                        roleInDiscipline[0].toUpperCase() +
                            roleInDiscipline.substring(1),
                  ...
                  builder: (context, levelsSnapshot) {
                    ...
                    final levelToUse =
                        allLevels.firstWhereOrNull(
                          (doc) => doc.id == currentLevelId,
                        ) ??
                        allLevels.first;
                    final currentLevelOrder =
                        (levelToUse.data() as Map<String, dynamic>)['order'];
                                      ...
                                      onPressed: () async {
                                        final confirm =
                                            await showDialog<bool>(
                                              ...
                                              builder: (ctx) => AlertDialog(
                                                title: Text(
                                                  l10n.revertPromotion,
                                                ),
                                                content: Text(
                                                  l10n.revertPromotionConfirm,
                                                ),
                                                actions: [
                                                  TextButton(
                                                    onPressed: () =>
